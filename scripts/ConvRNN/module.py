...
import torch
from torch.nn import Parameter, Conv2d, MaxPool2d
...
class ConvNdRNNBase(torch.nn.Module):
    def __init__(self,
        ...
        self.conv1 = Conv2d(self.out_channels, 256, 7, padding=3)
        self.conv2 = Conv2d(256, 256, 5, padding=2)
        self.conv3 = Conv2d(256, 256, 3, 1, 1)
        self.conv4 = Conv2d(256, 32, 2, 2)
        self.conv5 = Conv2d(32, 1, 2, 2)
        self.maxpool1 = MaxPool2d(2, 1, 1)
    ...
    def forward(self, input, hx=None):
        ...
        output = output[:, -1, :, :, :]
        # output = output.reshape(1024, 96, 11, 11)
        output = torch.relu(self.conv1(output))
        output = torch.relu(self.conv2(output))
        output = torch.relu(self.maxpool1(output))
        output = torch.relu(self.conv3(output))
        output = torch.relu(self.maxpool2(output))
        output = torch.relu(self.conv4(output))
        output = torch.relu(self.conv5(output))
